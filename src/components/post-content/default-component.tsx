...
const defaultComponents = {
    ...
    h3:headingBold,
    p({node, ...props}){
        const embed = node.children.find((child) => child.tagName === "tweet" || child.tagName === "codepen" || child.tagName === "guestBookForm") 
...
codepen({node, ...props}){
    return (
        <iframe
        height="300"
        className="m-0 mb-6 max-v-w[100vw] bleed transform relative left-1/2 -translate-x-1/2"
        scrolling="no"
        title={props.title}
        src={`https://codpen.io/dr3apap/embed/preview/${props.id}?default-tab=result&editable=true&theme-id=43641`}
        frameBorder="no"
        loading="lazy"
        allowTransparency
        allowFullScreen
        >Check out the <a href={`https://codepen.io/dr3apap/${props.id}`}>demo pen</a>by Adebola (<a href="https://codepen.io/dr3apap">@dr3apap</a>) over on{' '}<a href={`https://codepen.io/dr3apap/${props.id}`}>Codepen</a>.</iframe>
    )
},

tweet({node, account='dr3_147', ...props}){
    return (
        <div className="mb-6 rounded-lg grid place-items-center">
            <blockquote className="twitter-tweet" data-theme="dark">
                <a href={`https://twitter.com/${account}`}>Check out this related tweet!</a>
            </blockquote>
            <script>
            async 
            src="https://platform.twitter.com/widget.js"
            charset="utf-8"
            </script>
        </div>
    )
...
details({node, ...props}){
    return (
        <details className="bg-brand-light-surface1 mb-4">{props.children}</details>
    )
},
summary({ node, ...props }) {
...
ul({ node, className, children, ...props }) {
  ...
  pre({ node, ...props }) {
    return (
      <pre tabIndex={0} className="rounded-lg bg-[var(--gray-8)] p-4 m-0 overflow-auto mb-6">
        {props.children}
      </pre>
    )
  },
