...
const Nav = () => { 
		...
		<ul id="nav-toggle" aria-expanded="false" role-label="nav-list" className="col-[1_/-1] flex gap-2 max-w-screen-la md:col-[2_/3] mx-auto lg:translate-x-[35px] lg:w-[768px]">
            ...
            {["about-me", "demos", "Blog", "Contact"].map((route, index) => {return  <li className="text-fluid-0 text-center " key={index}><a className="block p-2 hover:bg-hover-light text-text-2-accent font-bold text-fluid-0" href={index > 1?`/${route.toLowerCase()}`:`#${route}`}>{index == 0?"About":index == 1?"Demos":route}</a></li>})}
		</ul>
		</nav>
    <ThemeToggle/>
	</div>
