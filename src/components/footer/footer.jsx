    ...
    <footer className="mv-0 mx-auto max-w-screen-md py-8 text-text-2">
      <div className="text-fluid--2 grid place-items-center gap-y-2 w-full">
        ...
        <ul className="font-bold flex gap-x-2 self-start">
          {['/', 'about-me', 'demos', 'Blog'].map((route, index) => {
            return (
              <li className="hover:bg-surface-3 text-center p-1" key={index}>
                <a
                  className="block p-2 hover:bg-surface-3 text-text-2-accent font-bold text-fluid-0 decoration-4 decoration-brand"
                  href={index > 2 ? `/${route.toLowerCase()}` : `#${route}`}
                >
                  {index == 0
                    ? 'Home'
                    : index == 1
