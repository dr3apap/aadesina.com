...
function BlogHeader({ bannerDemo, bannerAlt, heroImageSrc, heroImageAlt, character }) {
    return (
        <header className="w-content max-w-full mx-auto">
            <div className="w-full aspect-[3/1] bg-surface-4">
                ...
                {bannerDemo && bannerAlt && (
                    <iframe
                        title={bannerAlt} className="w-full h-full"
                        loading="lazy"
                        src={bannerDemo}
                    ></iframe>
                )}
            </div>
            <div className="w-article max-w-full mv-0 mx-auto grid mb-14">
                <div className="relative flex justify-end items-center min-h-half-avatar py-2">
                    <img
                        className="absolute top-0 transform -translate-y-1/2 left-4 md:left-0 rounded-full bg-surface-3 aspect-square border-4 border-text-1 w-avatar"
                        src={character?.image || "./asset/dr3-no-background-2.png"}
                        alt="dr3"
                        height="200"
                        width="200"
                        loading="eager"
