const { SANTIY_STUDIO_PROJECT_ID, SANITY_STUDIO_PROJECT_DATASET, PAGINATION_SIZE} = import.meta.env;
...
return `https://${SANTIY_STUDIO_PROJECT_ID}.api.sanity.io/v2021-03-25/data/query/${SANITY_STUDIO_PROJECT_DATASET}?query=${queryString}}`
}

export const getData = async (query:Query) =>{
const QUERY_URL = getQueryUrl(query);
const DATA = (await(await fetch(QUERY_URL)).json()).result;
return DATA;
}
