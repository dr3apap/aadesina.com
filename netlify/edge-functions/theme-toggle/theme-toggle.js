...
export default async (request, context) => {
  ...
  if (!contentType.startsWith('text/html') || request.url.includes('/demos/'))
    return
  ...
  // system -> light
  const afterNextTheme =
    THEME_STATE.at(THEME_STATE.indexOf(nextTheme) + 1) || THEME_STATE[0]
  ...
  if ((url = request.url).includes('theme-toggle')) {
    ...
    if (isClient) {
      ...
      return new Response(
        JSON.stringify({ theme: nextTheme, nextTheme: afterNextTheme }),
        {
          headers,
          status: 200,
        }
